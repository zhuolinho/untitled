...
/* Test uname. */
  ...
  if (access("/etc/uname", R_OK) != 0) {
	printf("/etc/uname is not present.  Cannot test uname()\n");
	errct++;
	return;
  }
  if (uname(&u) != 0) e(1);
  if (strncmp(u.sysname, "MINIX", 5) != 0) e(2);    /* only one defined */
}
...
/* Test getenv.  Asume HOME, PATH, and LOGNAME exist (not strictly required).*/
  ...
  if (*p != ':') e(4);		/* this is a MINIX convention, not POSIX */
