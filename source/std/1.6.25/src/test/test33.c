...
{				/* Test errors returned. */
  ...
  System("mkdir nosearch");
  System("> nosearch/file");
  System("> file");
  Chmod("nosearch/file", 05777);
  Chmod("file", 05777);
  Chmod("nosearch", 0677);
  if (!superuser) {
	if (access("nosearch/file", F_OK) != -1) e(17);
	if (errno != EACCES) e(18);
	if (access("nosearch/../file", F_OK) != -1) e(19);
	if (errno != EACCES) e(20);
  }
  if (superuser) {
	if (access("nosearch/file", F_OK) != 0) e(21);
  }

  /* Double check the file is accessable. */
  Chmod("nosearch", 0700);
  if (access("nosearch/file", F_OK) != 0) e(22);
