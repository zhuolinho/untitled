...
_PROTOTYPE(void test8p, (void));
_PROTOTYPE(void test8q, (void));
_PROTOTYPE(void clearsigstate, (void));
_PROTOTYPE(void quit, (void));
_PROTOTYPE(void wait_for, (pid_t pid));
...
/* Test sigprocmask and sigpending. */
  ...
  */
  ...
  for (i = 0; i < SIGS; i++) sigaction(i, (struct sigaction *) 0, &osa);
  ...
  if (osa.sa_flags != (SA_NODEFER | SA_NOCLDSTOP)) e(16);
  ...
  if (sigprocmask(SIG_SETMASK, (sigset_t *) 0, &s1) != 0) e(49);
  if (s1 != s_nokill) e(50);
  if (sigprocmask(SIG_SETMASK, &s_ill_pip, &s1) != 0) e(51);
  if (s1 != s_nokill) e(52);
  if (sigprocmask(SIG_SETMASK, (sigset_t *) 0, &s1) != 0) e(53);
  if (s1 != s_ill_pip) e(54);
  if (sigprocmask(SIG_BLOCK, (sigset_t *) 0, &s1) != 0) e(55);
  if (s1 != s_ill_pip) e(56);
  if (sigprocmask(SIG_UNBLOCK, (sigset_t *) 0, &s1) != 0) e(57);
...
int signo;
{
  if (sigprocmask(SIG_BLOCK, (sigset_t *)NULL, (sigset_t *) & glo_vol_set) != 0) e(1);
}
...
void test8c()
{
  ...
  /* If you have compiled signal() with the BSD behaviour, then
   * the next line will incorrectly detect an error.
   */
...
int signo;
{
  ...
  if (sigemptyset(&set) == -1) e(1);
  if (sigaddset(&set, SIGTERM) == -1) e(2);
  if (sigaddset(&set, SIGHUP) == -1) e(3);
  if (sigaddset(&set, SIGINT) == -1) e(4);
  if (sigaddset(&set, SIGPIPE) == -10) e(5);
  if (sigprocmask(SIG_BLOCK, (sigset_t *)NULL, &oset) != 0) e(6);
  if (oset != set) e(7);
}
...
void test8f()
{
  ...
  switch (pid = fork()) {
	...
	if (sigaction(SIGINT, &act, 0) == -1) e(4);
...
void test8g()
{
  ...
  switch (pid = fork()) {
	...
	if (sigaction(SIGPIPE, &act, 0) == -1) e(5);
...
void test8h()
{
  ...
  switch (pid = fork()) {
	...
	if (sigaction(SIGHUP, &act, 0) == -1) e(7);

	act.sa_handler = sigpip8;
	if (sigaction(SIGPIPE, &act, 0) == -1) e(8);

	act.sa_handler = sigter8;
	if (sigaction(SIGTERM, &act, 0) == -1) e(9);
...
void test8i()
{
  ...
  if (sigaction(SIGHUP, &act, 0) == -1) e(2);

  act.sa_handler = sigter9;
  if (sigaction(SIGTERM, &act, 0) == -1) e(3);
...
*
* Set an alarm for 6 second, then sleep for 7.
*
...
void test8j()
{
  ...
  if (sigaction(SIGALRM, &act, 0) == -1) e(3);
...
/* Basic test for _setjmp/_longjmp. */
...
void test8o()
{
  ...
  act.sa_flags = 0;
  act.sa_mask = 0;
  act.sa_handler = (sighandler_t) catch14;
